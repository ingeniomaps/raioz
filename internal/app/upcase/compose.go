...
func (uc *UseCase) processCompose(ctx context.Context, deps *config.Deps, ws *interfaces.Workspace, projectDir string) (string, []string, []string, error) {
	...
	if err != nil {
		...
		return "", nil, nil, errors.New(errors.ErrCodeWorkspaceError, i18n.T("error.compose_generate_failed")).WithSuggestion(i18n.T("error.compose_generate_suggestion")).WithError(err)
	}
	...
	if len(infraNames) > 0 {
		...
		if err := uc.deps.DockerRunner.UpServicesWithContext(ctx, composePath, infraNames); err != nil {
			...
			return "", nil, nil, errors.New(errors.ErrCodeDockerNotRunning, i18n.T("error.infra_start_failed")).WithSuggestion(i18n.T("error.infra_start_suggestion")).WithContext("compose_file", composePath).WithError(err)
		}
	...
	if len(serviceNames) > 0 {
		...
		if err := uc.deps.DockerRunner.UpServicesWithContext(ctx, composePath, serviceNames); err != nil {
			...
			return "", nil, nil, errors.New(errors.ErrCodeDockerNotRunning, i18n.T("error.services_start_failed")).WithSuggestion(i18n.T("error.services_start_suggestion")).WithContext("compose_file", composePath).WithError(err)
		}
