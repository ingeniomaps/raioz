...
func TestCaddyfileGeneration_GlobalOptions(t *testing.T) {
	...
	if !strings.Contains(content, "auto_https disable_certs") {
		t.Error("expected auto_https disable_certs for mkcert mode (stops ACME but keeps the HTTP→HTTPS redirect)")
	}
...
func TestCertsDir_ReturnsAbsolute(t *testing.T) {
	...
	// Normalize the OS separator so the same suffix check works
	// on Windows (filepath.Join emits `\`).
	if !strings.HasSuffix(filepath.ToSlash(dir), ".raioz/certs") {
...
func TestCaddyfileGeneration_NoGlobalOptionsWithoutCerts(t *testing.T) {
	...
	// would otherwise attempt ACME, which is the exact scenario this
