...
type RaiozConfig struct {
	...
	// WorkspaceRoot opts this config into a wider path-safety containment
	// boundary (ADR-036 H2). By default every path in the yaml must resolve
	// inside the yaml's own directory; in a multi-repo workspace where the
	// raioz.yaml lives in a sub-repo and services point at sibling repos via
	// `../`, that rejects a legitimate topology. Declaring workspaceRoot
	// (relative to this yaml, e.g. `..`) moves the containment boundary to
	// that directory: paths are still HARD-contained — just against the
	// declared root instead of baseDir. Empty = classic per-repo containment
	// (no behavior change). The declared root itself may not be a system dir.
	WorkspaceRoot string                    `yaml:"workspaceRoot,omitempty"` // since: v0.14.0
