...
| **`refcount.withLock`** | per-machine (`flock` on `.shared-deps.lock` + `processMu`) | The shared-dep refcount file's read-modify-write (ADR-050) | `AddRef` / `DropRef` / `Refs` / `Reconcile` | No — siblings included | self-contained: acquired and fully released within one call, never held across another lock |
