...
- **[ADR-037](docs/decisions/037-replaceable-edge-router.md)** — Workspace `router.project:` (meta config) replaces the bundled Caddy with a sibling raioz project that owns edge routing. The meta runner brings the router up first, sets `RAIOZ_ROUTER_ACTIVE=1` for every consumer sub-up (suppresses their Caddy via `internal/app/upcase/router_env.go`), and tears the router down last. `--router-off` on `raioz up` bypasses the branch for debugging. V1 ships no service-discovery contract — the router owns its own templates. **Trust asymmetry**: `router.project:` runs with strictly more trust than entries in `projects:` because it owns/sees all HTTP traffic for the workspace. Treat it as max-blast-radius — never declare as router a project whose source you don't fully control.
- **[ADR-041](docs/decisions/041-meta-orchestrator.md)** — `kind: meta` orchestrator (`internal/app/meta.go::MetaRunner`) is out-of-process, sequential, with a fixed per-sub-command failure matrix: `up` hard-fails on first non-optional failure; `down`/`status` are always best-effort. The meta runner takes no workspace/project lock — each child takes its own (`docs/LOCKS.md § "Meta runner sits outside both locks"`). Env propagation is the public contract for sub-spawns: `RAIOZ_ROUTER_ACTIVE`, `RAIOZ_SIBLING_STACK`, `RAIOZ_CORRELATION_ID` are added; everything else inherits from `os.Environ()` (see ADR-040 trust posture). Any change to parallelism, partial-failure tolerance, or env contract requires updating this ADR.
- **[ADR-042](docs/decisions/042-certs-stay-at-home-raioz.md)** — TLS certs stay at `~/.raioz/certs/<domain>/` (ADR-003 layout) — intentionally NOT migrated to `RaiozStateDir()`. Co-located with mkcert's CAROOT in `$HOME`. ADR-022's "single source of truth" applies to raioz-owned runtime state; certs are tool-managed (mkcert) artifacts, out of scope.
- **[ADR-043](docs/decisions/043-multi-machine-non-goal.md)** — Multi-machine workspaces are a non-goal for v1.0. raioz is single-host by construction (Docker network local, `host.docker.internal` discovery, per-machine state/locks/audit/certs). v2 candidate, no scheduled work.
- **[ADR-044](docs/decisions/044-tui-dashboard-observer-only.md)** — `raioz dashboard` (`internal/tui/`) reads Docker + local state and mutates through exactly three keys: `r` restart, `s` stop, `e` exec, each on one selected container. They take **no lock**, so they race a concurrent `up`/`down`/`--watch` — accepted, single-container, recoverable. Never writes state files, never spawns raioz, never IPC. A *fourth* action requires a separate ADR covering lock acquisition + concurrent-process coordination.
- **Every runner that owns a container injects `host.docker.internal:host-gateway` into `extra_hosts`.** `internal/discovery/discovery.go` hardcodes `host.docker.internal` as the container→host bridge. Runners that create a compose overlay (`internal/orchestrate/{compose,image,dockerfile}_runner.go`) and the proxy (`internal/proxy/proxy.go`) MUST inject the host-gateway mapping so the bridge resolves under Linux without Docker Desktop. See [ADR-047 § H-1](docs/decisions/047-hygiene-rules-catalog.md#h-1-host-gateway-alias-in-extra_hosts).
- **State file writes are atomic.** Every write under `RaiozStateDir()` or `<projectDir>/.raioz.state.json` must go through `fsutil.WriteFileAtomic` (`internal/fsutil/atomic.go`) — never direct `os.WriteFile`. A SIGKILL mid-write would otherwise leave a zero-byte or partial JSON that `LoadLocalState` cannot parse, blocking the project until manual `rm`. The `proxy/routes_persist.go` predates this rule and uses its own helper; future writers go through `fsutil`. See [ADR-047 § H-2](docs/decisions/047-hygiene-rules-catalog.md#h-2-atomic-state-writes).
